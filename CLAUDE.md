...
TKR is a Salesforce DX project implementing an account/order management application with Lightning Web Components (LWC) for the frontend and Apex for backend business logic. It demonstrates platform events, GraphQL API integration, and component composition patterns.
...
### Code Quality
```bash
npm run lint                # Lint all Aura/LWC JavaScript
...
### Testing
```bash
npm run test                  # Run all LWC Jest tests
npm run test:unit:watch       # Watch mode
npm run test:unit:debug       # Debug mode
npm run test:unit:coverage    # Coverage report
```

### Salesforce CLI (run directly, not npm-scripted)
```bash
sfdx org:create --definition config/project-scratch-def.json
sfdx source:push        # Push local changes to org
sfdx source:pull        # Pull changes from org
sfdx source:deploy --source-dir force-app
```
...
- `classes/` — Apex backend controllers and handlers
- `triggers/` — Apex triggers (delegate to handler classes)
- `lwc/` — Lightning Web Components
- `objects/` — Custom object/field definitions (Menu_Item__c, Order__c, Order_Item__c, Account_Dashboard_Event__e platform event)

### Backend (Apex)

Controllers expose `@AuraEnabled` methods; handler classes contain business logic.

Key classes:
- `AccountController` — Basic account queries with caching
- `getAccountDetails` — Aggregates account + contacts + opportunities + cases into an `AccountWrapper` inner class; illustrates the wrapper pattern used throughout
- `opportunityTriggerHandler` — Maintains denormalized `Account.Total_Opportunity__c`; compare the Map-based vs aggregate SOQL approaches in this file
- `contactDetails` — Contact-scoped case retrieval

Trigger pattern: `accountTrigger` publishes `Account_Dashboard_Event__e` on insert; `opportunityTrigger` delegates to the handler for rollup updates.

### Frontend (LWC)

Key components:
- `accountDetailsComponent` — Master/detail view of account + related datatables, driven by `@wire` + Apex
- `accountWithEventRefresh` — Subscribes to `Account_Dashboard_Event__e` platform events to reactively refresh
- `menuTKR` — GraphQL-driven (UI API) component querying Menu_Item__c; the only component using `gql` template literals instead of Apex wiring
- `datatableActions` — Reusable datatable with row-level actions
- `parentComponent`/`childComponent` — Compositional event-passing examples

### Data Flow

1. User opens an account record → `accountDetailsComponent` fires `@wire(getAccountDetailsOnly, { recordId })`.
2. Apex aggregates and returns `AccountWrapper` (account + related lists).
3. On opportunity insert/update → `opportunityTrigger` → `opportunityTriggerHandler` updates `Account.Total_Opportunity__c`.
4. `accountTrigger` publishes `Account_Dashboard_Event__e`.
5. `accountWithEventRefresh` receives the event and triggers a component refresh.

## Code Conventions

### Apex
- Class headers: Author, Created Date, Last Modified, Change History.
- Use `@AuraEnabled(cacheable=true)` for read-only methods; `cacheable=false` for data that changes frequently.
- Return complex object graphs via inner wrapper classes, not `Map<String, Object>`.


### LWC
- Prefer `@wire` over imperative Apex calls for reactive data binding.
- GraphQL queries use `gql` tagged template literals (see `menuTKR`); all other components use `@salesforce/apex/` imports.
- Column definitions are class properties on the component, not inline in the template.

### Formatting
- Prettier with `prettier-plugin-apex` and `@prettier/plugin-xml`; no trailing commas (`.prettierrc`).
- LWC HTML templates use `parser: "lwc"` override in Prettier config.
- Pre-commit hook (Husky + lint-staged) runs ESLint, Prettier, and Jest automatically.
